...
function authHarness(overrides = {}) {
  ...
  const api = {
    ...
    reauthenticateWithCredential: async (_, credential) =>
      calls.push(["reauth", credential]),
    verifyBeforeUpdateEmail: async (_, email) =>
      calls.push(["verify-new-email", email]),
    updatePassword: async (_, password) => calls.push(["password", password]),
    ...overrides,
  };
  return {
    calls,
    service: load("src/features/UserProfile/services/auth-service.ts", {
      "@/services/firebase/config": { auth: { currentUser: user } },
      "firebase/auth": api,
    }),
  };
}
test("Name-only edit does not request reauthentication or email verification", async () => {
  ...
  assert.equal(
    await service.editProfile(" Maria ", "old@example.com", ""),
    false,
  );
...
test("Email change requires current password and verifies before changing the address", async () => {
  ...
  await assert.rejects(
    service.editProfile("Maria", "new@example.com", ""),
    /senha atual/,
  );
  assert.equal(calls.length, 0);
  assert.equal(
    await service.editProfile("Maria", "new@example.com", "current"),
    true,
  );
  assert.deepEqual(
    calls.map((c) => c[0]),
    ["reauth", "verify-new-email", "profile"],
  );
});
test("Wrong credentials do not mutate profile, email or password", async () => {
  ...
  await assert.rejects(
    service.editProfile("Maria", "new@example.com", "wrong"),
    /invalid-credential/,
  );
  await assert.rejects(
    service.changePassword("wrong", "new-password"),
    /invalid-credential/,
  );
  assert.equal(calls.length, 0);
});
...
test("Denied notifications never persist an enabled preference", async () => {
  ...
  await assert.rejects(
    service.setNotificationsEnabled("one", true),
    /Permissão negada/,
  );
...
test("Native opt-in schedules one daily reminder; disabling cancels it", async () => {
  ...
  assert.equal(schedule.trigger.hour, 20);
  assert.equal(schedule.trigger.type, "daily");
