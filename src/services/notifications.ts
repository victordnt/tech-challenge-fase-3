...
export async function setNotificationsEnabled(
  ...
  if (enabled) {
    ...
    if (!permission.granted)
      throw new Error(
        "Permissão negada. Ative as notificações nas configurações do aparelho e tente novamente.",
      );
    await Notifications.cancelScheduledNotificationAsync(identifier);
    await Notifications.scheduleNotificationAsync({
      identifier,
      content: {
        title: "Seu momento Mavi",
        body: "Reserve um minuto para atualizar suas entradas e saídas.",
      },
      trigger: {
